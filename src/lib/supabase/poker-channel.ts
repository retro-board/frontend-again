...
import type { PokerChannelMessage, PokerSessionState } from "~/types/poker-channel";
...
export class PokerChannelClient {
  ...
  private messageHandlers: Map<string, (message: PokerChannelMessage) => void> = new Map();
  ...
  async vote(storyId: string, voteValue: string, userId?: string, anonymousUserId?: string, userName?: string) {
  ...
  async join(role: "facilitator" | "voter" | "observer", userId?: string, anonymousUserId?: string, userName?: string) {
    await this.sendMessage({
      type: "join",
      sessionId: this.sessionId,
      role,
  ...
  async unabstain(userId?: string, anonymousUserId?: string, userName?: string) {
    await this.sendMessage({
      type: "unabstain",
      sessionId: this.sessionId,
      userId,
  ...
  async createStory(story: { id: string; title: string; description?: string; position: number }, userId: string) {
    await this.sendMessage({
      type: "story_create",
      sessionId: this.sessionId,
      userId,
  ...
  async announceScore(storyId: string, finalScore: string, votes: Record<string, string>, userId: string) {
    await this.sendMessage({
