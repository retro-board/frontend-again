...
import type { Story, PokerParticipant, User } from "~/types/database";
...
export function usePokerChannel({
  ...
  const handleChannelMessage = useCallback((message: PokerChannelMessage) => {
    setSessionState((prev) => {
      ...
      switch (message.type) {
        case "join": {
          const participantId = message.userId || message.anonymousUserId || "unknown";
  ...
  const createStory = useCallback(
    async (story: { id: string; title: string; description?: string; position: number }) => {
      if (!channelRef.current || !user?.id) return;
  ...
  const announceScore = useCallback(
    async (storyId: string, finalScore: string, votes: Record<string, string>) => {
      if (!channelRef.current || !user?.id) return;
      await channelRef.current.announceScore(storyId, finalScore, votes, user.id);
    },
    [user]
  );
