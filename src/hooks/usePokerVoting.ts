...
export function usePokerVoting({
  ...
  const checkVotingCompletion = useCallback(async () => {
    ...
    const response = await fetch(`/api/poker/${sessionId}/voting?storyId=${currentStoryId}`);
    if (response.ok) {
  ...
  }, [sessionId, currentStoryId, hasVoted, isAnonymous, anonymousUserId, sendVote, checkVotingCompletion]);
  ...
  }, [sessionId, isAbstaining, isAnonymous, anonymousUserId, sendAbstain, sendUnabstain]);

  // Facilitator: Start voting on a story
  const startVoting = useCallback(async (storyId: string, timerDuration?: number) => {
  ...
  }, [sessionId, isFacilitator, channelSelectStory, channelStartVoting, channelStartTimer, timer]);

  // Facilitator: End voting and calculate score
  const endVoting = useCallback(async (storyId: string) => {
    if (!isFacilitator) return;
  ...
  }, [sessionId, isFacilitator, channelEndVoting, channelAnnounceScore, channelStopTimer, timer]);
